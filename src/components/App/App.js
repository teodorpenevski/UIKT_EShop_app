...
class App extends Component{
        ...
        return (
            <Router>
                ...
                <div className={"container"}>
                    <Route exact path="/monitors/addMonitor" render={() =>
                        <MonitorAdd categories={this.state.categories}
                                    onAddMonitor={this.addMonitor}/>}/>
                    <Route exact path="/mice/addMice" render={() =>
                        <MouseAdd categories={this.state.categories}
                                  onAddMouse={this.addMouse}/>}/>
                    <Route path="/keyboards/addKeyboard" exact render={() =>
                        <KeyboardAdd categories={this.state.categories}
                                     onAddKeyboard={this.addKeyboard}/>}/>
                    <Route path="/laptops/addLaptop" exact render={() =>
                        <LaptopAdd categories={this.state.categories}
                                   onAddLaptop={this.addLaptop}/>}/>
                    <Route exact path="/products/edit/:id"  render={() =>
    ...
    addMonitor = (name, price, back_light_type, color, power_in_kw, resolution, category) => {
        EShopService.addMonitor(name, price, back_light_type, color, power_in_kw, resolution, category)
            .then(() => {
                this.loadProducts();
            });
    }

    addMouse = (name, price, dpiresolution, cable_length, cable_type, is_gaming, category) => {
        EShopService.addMouse(name, price, dpiresolution, cable_length, cable_type, is_gaming, category)
            .then(() => {
                this.loadProducts();
            });
    }
    ...
    addKeyboard = (name, price, spill_resistant, is_gaming,
        ...
        EShopService.addKeyboard(name, price, spill_resistant, is_gaming,
            cable_length_in_cm, number_of_keys, category)
            .then(() => {
                this.loadProducts();
            });
    ...
    addLaptop = (name, price, cpu, gpu, ram, dimensions, operating_system, storage, weight_in_kg, category) => {
        EShopService.addLaptop(name, price, cpu, gpu, ram, dimensions, operating_system,
            storage, weight_in_kg, category)
            .then(() => {
                this.loadProducts();
            });
    }
    ...
    editProduct = (id, price) => {
        EShopService.editProduct(id, price)
            .then(() => {
                this.loadProducts();
            });
    }
